...
```bash
git remote add origin https://github.com/PhoenixWeaver/BinhBenTran_RF_CV.git
```

Or if the remote already exists, update it:
```bash
git remote set-url origin https://github.com/PhoenixWeaver/BinhBenTran_RF_CV.git
```
...
1. Go to your repository: https://github.com/PhoenixWeaver/BinhBenTran_RF_CV
2. Click on **Settings**
...
```bash
cd D:\CV && git init && git remote add origin https://github.com/PhoenixWeaver/BinhBenTran_RF_CV.git && git add . && git commit -m "Add interactive RF & Microwave Engineer CV" && git branch -M main && git push -u origin main
```
